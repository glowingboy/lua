...
static int math_modf (lua_State *L) {
  float ip;
  float fp = modff(luaL_checknumber(L, 1), &ip);
  lua_pushnumber(L, ip);
  lua_pushnumber(L, fp);
  return 2;
}
...
static int math_pow (lua_State *L) {
  lua_pushnumber(L, powf(luaL_checknumber(L, 1), luaL_checknumber(L, 2)));
  return 1;
}
...
static int math_random (lua_State *L) {
  ...
  switch (lua_gettop(L)) {  /* check number of arguments */
    ...
    case 1: {  /* only upper limit */
      ...
      lua_pushnumber(L, floorf(r*u)+1);  /* int between 1 and `u' */
      break;
    }
    case 2: {  /* lower and upper limits */
      ...
      lua_pushnumber(L, floorf(r*(u-l+1))+l);  /* int between `l' and `u' */
      break;
    }
